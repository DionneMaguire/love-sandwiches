...
def validate_data(values):
    """
    inside thr try, converts all string values into integers.
    ...
    try:
        ...
        if len(values) != 6:
            raise ValueError(
                f"Exactly 6 values required, you provded {len(values)}"
            )
...
def update_surplus_worksheet(data):
    """
    update surplus worksheet with calculated surpluys figures
    """
